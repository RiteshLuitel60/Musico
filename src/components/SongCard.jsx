...
const SongCard = ({ song, isPlaying, activeSong, data, i }) => {
  ...
  const getSongTitle = () =>
    song?.attributes?.name || song?.title || "Unknown Title";
  const getSongId = () =>
    song?.hub?.actions?.[0]?.id || song?.id || "default-id";
  const getArtistId = () =>
  ...
  const getCoverArt = () =>
    ...
    "default-image-url";
  const getActiveSongComparator = () =>
    song.attributes?.name || song.key || "default-comparator";
  const getArtistName = () =>
    song.attributes?.artistName || song.subtitle || "Unknown Artist";

  return (
    <div className="flex flex-col w-[140px] sm:w-[180px] p-2 bg-white/5 bg-opacity-80 backdrop-blur-sm animate-slideup rounded-lg cursor-pointer">
      ...
      <div className="mt-2 flex flex-col">
        ...
        <p className="text-xs sm:text-sm truncate text-gray-300 mt-1">
          <Link
            to={getArtistId() ? `/artists/${getArtistId()}` : "/top-artists"}
          >
            {getArtistName()}
          </Link>
        </p>
